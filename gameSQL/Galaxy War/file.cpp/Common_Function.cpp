...
bool SDLCommonFunc::CheckFocusWithRect(const int&x , const int &y,const SDL_Rect & rect)       //nhấn vào khoảng chữ play game, 
{
...
{
	g_img_menu = LoadImage("cccc.png");               //anh menu game
	if(g_img_menu == NULL)
	{
		return 1;
	}
...
SDL_Surface* SDLCommonFunc::LoadImage(std::string file_path)
{
	...
	SDL_FreeSurface(load_image);
		...
		UINT32 color_key1 = SDL_MapRGB(optimize_image->format,0,0,0);
...
SDL_Rect SDLCommonFunc::ApplySurface(SDL_Surface* src, SDL_Surface* des, int x, int y)        // lấy toạ độ ảnh load 
{
  SDL_Rect offset;
  offset.x = x;
  offset.y = y;
  SDL_BlitSurface(src, NULL, des, &offset);            //sao chép src vào des
  return offset;
}

void SDLCommonFunc:: ApplySurfaceVideo(SDL_Surface* src, SDL_Surface* des, SDL_Rect*clip ,int x, int y)
{
  SDL_Rect offset;
  offset.x = x;
  offset.y = y;
